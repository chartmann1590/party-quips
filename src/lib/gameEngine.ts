...
function assignQuiplashPrompts(
  ...
  if (round === ROUNDS_TOTAL) {
    const finalPrompt = getRandomFinalLashPrompt(usedIds)
  ...
  const prompts = getRandomQuiplashPrompts(n, usedIds)
...
export async function startQuiplashGame(
  ...
  const assignments = assignQuiplashPrompts(players, round, usedPromptIds)
...
export async function startNextRoundOrFinish(
  ...
  await startQuiplashGame(code, players, nextRound, usedPromptIds)
  return 'next_round'
}

// ── Fibbage Engine ────────────────────────────────────────────────────────────

export async function startFibbageGame(code: string, round: number): Promise<string[]> {
  const prompts = getRandomFibbagePrompts(3)
...
export async function startTriviaRound(code: string, round: number): Promise<void> {
  const [question] = getRandomTriviaQuestions(1)
...
export async function advanceTriviaOrFinish(code: string, round: number): Promise<'next_round' | 'game_over'> {
  if (round >= TOTAL_TRIVIA_ROUNDS) {
    await setRoomState(code, 'scoreboard')
    return 'game_over'
  }
  await update(ref(db, `rooms/${code}/meta`), { round: round + 1 })
  await startTriviaRound(code, round + 1)
  return 'next_round'
}
